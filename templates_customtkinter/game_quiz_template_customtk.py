...
## NEW ##
# Set apperaance mode and colour theme 
ctk.set_appearance_mode("System")
ctk.set_default_color_theme("blue")

## NEW ##
# Using ctk.CTk instead of tk.Tk throughout
...
class IntroPage(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller): 
        ...
        # Title label 
        title_label = ctk.CTkLabel(self, text = "Welcome to the <Name> Quiz Game!", font=ctk.CTkFont("Helvetica, 14"))
        title_label.pack(pady=20)

        # Start button 
        start_button = ctk.CTkButton(self, text = "Start Quiz", command = lambda: controller.show_frame(MainPage))
...
class MainPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        spain_capital_label = ctk.CTkLabel(self, text = "What is the capital of Spain?")
        spain_capital_label.grid(row=current_row, column = 0, pady=20)
        current_row += 1

        self.capital_input_var = tk.StringVar(value="")
        capital_entry = ctk.CTkEntry(self, textvariable=self.capital_input_var)
        capital_entry.grid(row=current_row,column=0)
        current_row += 1

        # 2. Multiple choice (Is it normally cold in winter?)
        winter_cold_label = ctk.CTkLabel(self, text="Is it normally cold in winter?")
        winter_cold_label.grid(row=current_row, column=0,pady=20)
        current_row += 1

        self.winter_yes_no_option_state = ctk.IntVar(value=0)
        winter_yes_option = ctk.CTkRadioButton(self,text="Yes",value = 1, variable=self.winter_yes_no_option_state)
        winter_yes_option.grid(row=current_row,column=0)
        current_row += 1

        winter_no_option = ctk.CTkRadioButton(self,text="No",value = 2, variable=self.winter_yes_no_option_state)
        winter_no_option.grid(row=current_row,column=0)
        current_row += 1

        # 3. Another Input Entry (What species is a lizard?)
        species_label = ctk.CTkLabel(self, text="What species is a lizard?")
        species_label.grid(row=current_row, column=0,pady=20)
        current_row += 1

        self.species_var = ctk.StringVar(value = "")
        species_entry = ctk.CTkEntry(self,textvariable=self.species_var)
        species_entry.grid(row=current_row, column=0, pady=20)
        current_row += 1


        # Next button
        # The submit_choice function will call WinLosePage and pass in the data from the MainPage question
        next_button = ctk.CTkButton(self, text = "Next", command = self.submit_choice_and_show_result)
        #next_button.pack()
...
class ScorePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        # Label for result
        self.result_title_label = ctk.CTkLabel(self, text = 'Score:')
        self.result_title_label.pack(pady=(50,15))
        # Label for the score result
        self.result_message_label = ctk.CTkLabel(self, text="[Result]")
        self.result_message_label.pack(pady=(0,20))
        # Button to go back to Intro page
        back_to_intro = ctk.CTkButton(self, text = "Start Again", command = lambda: controller.show_frame(IntroPage))
    ...
    def display_result(self, result_text):
        ...
        self.result_message_label.configure(text=result_text)
